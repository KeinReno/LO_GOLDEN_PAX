...
export function SystemCommandPanel({
  ...
  const stationDefs = useMemo(() => {
    const catalog = getCachedContent()?.stations as
      | Record<
          string,
          {
            kind?: StationKind;
        ...
        >
      | undefined;
    const merged: StationCat[] = DEFAULT_STATIONS.map((d) => {
      const fromCat =
        catalog?.[d.kind] ||
        Object.values(catalog || {}).find((s) => s.kind === d.kind);
      return {
        ...d,
        name: fromCat?.name || d.name,
        ap: Number(fromCat?.ap ?? d.ap),
        cost: fromCat?.cost || d.cost,
        requireProperties: fromCat?.requireProperties,
      };
    });
    if (!preferKind) return merged;
    return [
      ...merged.filter((d) => d.kind === preferKind),
      ...merged.filter((d) => d.kind !== preferKind),
    ];
  }, [preferKind]);
