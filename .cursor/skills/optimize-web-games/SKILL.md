...
description: Profile, diagnose, and improve web game performance without regressing gameplay. Use for frame-time drops, CPU/GPU pressure, draw calls, texture and geometry budgets, animation loops, adaptive quality, mobile performance, and browser performance verification. Adapted for LO_GOLDEN_PAX: 2D canvas renderer (MapCanvas/drawMapIcons), not Three.js — read "GPU/draw calls" as canvas paint cost and "geometry" as icon/sprite draw.
---
...
For LO_GOLDEN_PAX: pick a representative late-game map (many colonies, active intents, combat) and record canvas size, icon count, intent count, pathfinding grid size, and economy tick cost.
...
- Suspect GPU/paint when resolution, overdraw, shadows, transparent particles, post-processing, draw calls, geometry, or texture bandwidth dominate.
...
For canvas: batch icon draws, avoid per-icon save/restore, cache static layers, throttle redraw when idle, and avoid re-creating gradients/textures per frame.
