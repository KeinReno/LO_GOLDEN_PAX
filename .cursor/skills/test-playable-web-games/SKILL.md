...
description: Test a playable browser game end to end with deterministic fixtures and real browser evidence. Use for gameplay QA, regression testing, controls, accessibility, responsive/mobile testing, save flows, console checks, performance smoke tests, and release verification. Directly applies to LO_GOLDEN_PAX GMap.
---
...
For LO_GOLDEN_PAX: journey = open viewer, select planet, dispatch intent, run economy tick, resolve combat, view RP/log, save world state. Add matrix rows per intent type and per faction.
