/**
 * Faction/race gating for building definitions. Ported verbatim from
 * GMap/server/buildingAccess.mjs — already pure. Parity verified in
 * buildingAccess.parity.test.mjs.
 */
...
export function canFactionBuildDef(def, factionId, opts = {}) {
  ...
  const factionOk = !def.faction || def.faction === "generic" || def.faction === tag;
...
export function raceIdsFromComposition(composition) {
  return (composition ?? []).map((r) => String(r.raceId || "").replace(/^race_/, ""));
}
