/**
 * Culture & faith effects — data-driven from content/core/cultures.json
 * and faiths.json. Ported verbatim from GMap/server/cultureFaith.mjs
 * (already pure), except its factionHasProperty import from
 * server/techActions.mjs, which is this project's domain/tech/
 * properties.mjs. Parity verified in cultureFaith.parity.test.mjs.
 */
import { factionHasProperty } from "../tech/properties.mjs";
...
function pushTraitEffects(out, traitList, sourceKind, sourceId, label, weight = 1) {
  for (const trait of traitList || []) {
    for (const eff of trait.effects || []) {
      out.push({ ...eff, source: { kind: sourceKind, id: `${sourceId}:${trait.id || sourceId}`, label }, _weight: weight });
    }
  }
}

function propertyLabel(content, propertyId) {
  return content?.economy_schema?.properties?.[propertyId]?.label ?? propertyId;
}
...
export function collectCultureEffects(content, cultureId, opts = {}) {
  ...
  if (Array.isArray(def.effects) && def.effects.length) {
    for (const eff of def.effects) out.push({ ...eff, source: { kind: "culture", id: def.id, label } });
  }
  ...
  if (opts.raceId && def.compatibleRaces?.length) {
    if (!def.compatibleRaces.includes(opts.raceId)) {
      out.push({ effect: "loyalty_add", args: { amount: -1 }, source: { kind: "culture", id: `${def.id}:mismatch`, label: `${label} (чужая раса)` } });
    }
  }
  return out;
}
 ...
 * @param {{ unlockedProperties?: string[] }} [techAccount]  see domain/tech/techAccount.mjs — GMap calls this param `eco`
 */
export function collectFaithEffects(content, faithShares, techAccount, opts = {}) {
  const out = [];
  for (const row of faithShares || []) {
    ...
    if (Array.isArray(def.effects)) {
      for (const eff of def.effects) {
        out.push({ ...eff, args: scaleWeightedArgs(eff, w), source: { kind: "faith", id: def.id, label } });
      }
    }
    for (const taboo of def.taboo_properties || []) {
      if (techAccount && !factionHasProperty(techAccount, taboo)) {
        out.push({
          effect: "production_mult",
          args: { mult: 1 - 0.05 * w },
          source: { kind: "faith_taboo", id: `${def.id}:${taboo}`, label: `${label}: табу ${propertyLabel(content, taboo)}` },
        });
      }
    }
  }
...
export function resolvePlanetFaithShares(planet, faction) {
  if (Array.isArray(planet?.faithShare) && planet.faithShare.length) return planet.faithShare;
  if (faction?.primaryFaith) return [{ faithId: faction.primaryFaith, percent: 100 }];
